...
export async function registerSkillsRoutes(
  ...
  // POST /skills
  app.post<{ Body: CreateSkillDto }>('/skills', { schema: createSkillSchema }, async (request, reply) => {
    const skill = await store.create(request.body)
    reply.code(201)
    return skill
  })
  ...
  app.get<{ Params: { id: string } }>('/skills/:id', async (request, reply) => {
    const skill = await store.get(request.params.id)
    if (!skill) {
      reply.code(404)
  ...
  app.put<{ Params: { id: string }; Body: UpdateSkillDto }>(
    ...
    async (request, reply) => {
      const skill = await store.update(request.params.id, request.body)
      if (!skill) {
        reply.code(404)
        return { error: 'Skill not found' }
      }
