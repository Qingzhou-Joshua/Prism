...
| **管理（Manage）** | 在同一个地方编辑规则、技能、Agent、MCP 服务器和 Profile |
...
Prism 目前管理跨平台的五种实体类型：

| 实体 | 功能 |
|------|------|
| **Rules（规则）** | 增删改查、Monaco 编辑器、各平台投影预览、从平台目录导入 |
| **Skills（技能）** | 增删改查、Monaco 编辑器、各平台投影预览、从平台目录导入 |
| **Agents（代理）** | 增删改查、Monaco 编辑器、各平台投影预览、从平台目录导入 |
| **MCP Servers** | 增删改查、表单编辑器、从 Claude Code settings.json 扫描并导入 |
| **Profiles（配置集）** | 组合规则/技能/Agent/MCP 服务器，带备份和版本历史的发布流程 |

发布流程支持 dry-run 预览、自动备份，并可通过「版本历史」标签页完整回滚。
...
│                          规则/技能/Agent/MCP/Profile/版本历史的完整 CRUD
│
apps/
└── web/                 # React + Vite 前端，Monaco 编辑器 + 投影预览
```
